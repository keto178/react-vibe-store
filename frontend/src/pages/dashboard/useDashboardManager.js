...
import { uploadAssetApi } from '../../api'
...
function buildReadOnlyMessage(serverHealth) {
    if (serverHealth?.writeAccess !== false) {
        return ''
    }

    return [serverHealth.message, serverHealth.setupHint]
        .filter((part) => typeof part === 'string' && part.trim())
        .join(' ') || 'Saving is disabled right now.'
}
...
export function useDashboardManager({
    ...
    const readOnlyMessage = buildReadOnlyMessage(serverHealth)
    ...
    const uploadSelectedFile = async (file, scope) => {
        ...
        const response = await uploadAssetApi(file, { scope })
    ...
    const handleCategorySubmit = async (event) => {
        ...
        try {
            const uploadResult = categoryForm.imageFile
                ? await uploadSelectedFile(categoryForm.imageFile, 'categories')
                : null
            const image = uploadResult?.url || categoryForm.existingImage
            const preparedCategory = {
                id: editingCategoryId || `${Date.now()}`,
                name: categoryForm.name.trim(),
                group: categoryForm.group,
                image,
                ...(uploadResult?.assetMetadata ? { imageMetadata: uploadResult.assetMetadata } : {})
            }
    ...
    const handleProductSubmit = async (event) => {
        ...
        try {
            const uploadResult = productForm.imageFile
                ? await uploadSelectedFile(productForm.imageFile, 'products')
                : null
            const image = uploadResult?.url || productForm.existingImage
