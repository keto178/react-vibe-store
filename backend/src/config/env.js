...
const ENV_MODULE_DIRECTORY = path.dirname(fileURLToPath(import.meta.url))
const BACKEND_ROOT_DIRECTORY = path.resolve(ENV_MODULE_DIRECTORY, '../..')
const WORKSPACE_ROOT_DIRECTORY = path.resolve(BACKEND_ROOT_DIRECTORY, '..')
const SYSTEM_ENV_KEYS = new Set(Object.keys(process.env))
...
function loadEnvFile(filePath) {
    ...
    const parsedEnvironment = dotenv.parse(readFileSync(filePath))
...
function loadEnvironmentFiles() {
    ...
    for (const environmentRoot of environmentRoots) {
        loadEnvFile(path.join(environmentRoot, '.env'))
    }

    const activeNodeEnv = String(process.env.NODE_ENV || '').trim() || 'development'
    const overrideFileNames = [
        `.env.${activeNodeEnv}`,
        '.env.local',
        `.env.${activeNodeEnv}.local`
    ]

    for (const fileName of overrideFileNames) {
        for (const environmentRoot of environmentRoots) {
            loadEnvFile(path.join(environmentRoot, fileName))
        }
    }
}
...
const env = {
    ...
    mongoFallbackRetryDelayMs: parsePositiveInt(process.env.MONGO_FALLBACK_RETRY_DELAY_MS, isProduction ? 30000 : 5000),
    ...
    adminPassword: normalizeString(process.env.ADMIN_PASSWORD) || PLACEHOLDER_ADMIN_PASSWORD,
